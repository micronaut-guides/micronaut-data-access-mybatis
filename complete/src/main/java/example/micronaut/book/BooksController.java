...
public class BooksController {
    ...
    public HttpResponse update(@Body @Valid BookUpdateCommand command) {
        Optional<Genre> genreOptional = Optional.ofNullable(genreRepository.findById(command.getGenreId()));
    ...
    HttpResponse<Book> save(@Body @Valid BookSaveCommand cmd) {
        Optional<Genre> genreOptional = Optional.ofNullable(genreRepository.findById(cmd.getGenreId()));
        return genreOptional.map(genre -> {
            Book book = new Book(cmd.getIsbn(), cmd.getName(), genre);
